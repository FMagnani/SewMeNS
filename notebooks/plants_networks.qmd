...
source(
  here::here("functions", "network_threshold_diagnosis.R"),
  local = netfun
)
```
...
assoc_list <- norm(plants_cph_common) %>%
  map(\(clr) cor(clr, method = "pearson", use = "pairwise.complete.obs"))
```

```{r}
## Threshold-dependent network analysis

thresholds <- seq(0.0, 1, by = 0.01)

cache_dir  <- "../cache"
cache_file <- file.path(cache_dir, "triplicate_prev0p25_mincount10_seed42_pearson.rds")
...
if (file.exists(cache_file)) {
  ...
  threshold_results <- readRDS(cache_file)
  ...
  saveRDS(threshold_results, cache_file)
  message("Diagnostics saved to: ", cache_file)
  
}

rm(thresholds, cache_file, cache_dir)
```
...
similarity_long <- similarity %>%
  mutate(comparison = paste(network1, network2, sep = " - ")) %>%
  pivot_longer(
    cols = c(edge_jaccard_signed, degree_cor, signed_strength_cor, community_ari),
    names_to = "metric", values_to = "value")

p_similarity_threshold <- ggplot(
  similarity_long,
  aes(x = threshold, y = value, color = comparison)
) +
  geom_line(linewidth = 0.8, alpha = 0.85, na.rm = TRUE) +
  geom_vline(xintercept = 0.85, linetype = "dashed", linewidth = 0.4) +
  facet_wrap(~ metric, scales = "free_y", ncol = 2) +
  labs(x = "Absolute correlation threshold",
       y = "Pairwise similarity", color = "Network pair") +
...
```{r}
diag_plot_tbl <- threshold_results$diagnosis %>%
  rename(plant = network) %>%
  select(
    plant, threshold,
...
```{r, fig.width=9, fig.height=3}
# ----------------------------
# Panel B: top-ranked taxa composition
# ----------------------------
top_rank_tbl <- taxa_tbl %>%
  ...
  ungroup() %>%
  mutate(
    Plant = factor(Plant, levels = c("Avedoere", "Damhusaaen", "Lynetten"))
  )

p_B <- ggplot(top_rank_tbl, aes(x = rank_eig, y = 1, fill = node_type)) +
  geom_tile(color = "white", linewidth = 0.25) +
  facet_wrap(~Plant, ncol = 1, scales = "free_y") +
  scale_x_continuous(
    breaks = c(1, 10, 20, 30, 40, 50, 75, 100)[c(1, 10, 20, 30, 40, 50, 75, 100) <= top_n],
    expand = c(0, 0)
  ) +
  labs(
    x = paste0("Rank by eigenvector centrality (top ", top_n, ")"),
    y = NULL,
    fill = NULL
  ) +
  theme_bw() +
...
plot_eig <- ggpubr::ggarrange(
  ...
  labels = c("A", "B"),
  common.legend = TRUE,
  legend = "bottom", label.y = c(1, 1.08)) 
...
```{r}
rm(assoc_list, diag_plot_tbl, taxa_tbl, top_rank_tbl,
   p_diag, p_A, p_B, plot_eig, top_n)
```
...
```{r}
rm(graph_list, pairwise_tbl, make_measurement_plot, panel)
...
plot_network_fixed_layout <- function(g, title = NULL) {
  ...
  as_tbl_graph(g) %>%
    activate(nodes) %>%
    mutate(
      comm_id = factor(comm_id,
                       levels = sort(unique(as.numeric(as.character(comm_id)))))) %>%
  ggraph(layout = lay_consensus) +
    geom_edge_link(
